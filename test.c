...
void test_stack_vma_offset(void) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        ASSERT_EQ(get_swap_offset_from_page(stack + (i * 4096)), 9-i);
        stack[i * 4096] = i;
    }
}
void test_stack_vma_enlarge(void) {
    ...
    *stack = 11;
    swapout_page(stack);
    ASSERT_EQ(get_swap_offset_from_page(stack), 10);
    ASSERT_EQ(*stack, 11);
    for (int i = 1; i < 11; i++) {
        swapout_page(stack + (i * 4096));
        ASSERT_EQ(get_swap_offset_from_page(stack + (i * 4096)), 10-i);
        ASSERT_EQ(stack[i * 4096], i-1);
    }
}
...
void test_vma_si_allcation(void) {
    ...
    for (int i = 0; i < 10; i++) {
        ASSERT_EQ(vma_has_swap_info(addr + (i * 4096)), 0);
    }
    swapout_page(addr);
    for (int i = 0; i < 10; i++) {
        ASSERT_EQ(vma_has_swap_info(addr + (i * 4096)), 1);
    }
}
...
void test_folio_offset(void) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        ASSERT_EQ(get_swap_offset_from_page(addr + (i * 4096)), i);
        ASSERT_EQ(addr[i], i);
    }
    for (int i = 0; i < 10; i++) {
        swapout_page(addr + (i * 4096));
...
void test_multiple_swapfiles(void) {
    ...
    for (int i = 0; i < 10; i++) {
        swapout_page(addr + (i * 4096));
        ASSERT_EQ(get_swap_offset_from_page(addr + (i * 4096)), i % 3);
        ASSERT_EQ(addr[i], i);
    }
